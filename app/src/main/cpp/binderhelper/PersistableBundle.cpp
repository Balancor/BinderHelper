...
#include <binderhelper/ParcelValTypes.h>

#include <limits>

#include <binderhelper/IBinder.h>
...
template <typename T>
bool getValue(const String16& key, T* out, const map<String16, T>& map) {
...
template <typename T>
set<String16> getKeys(const map<String16, T>& map) {
    if (map.empty()) return set<String16>();
    set<String16> keys;
