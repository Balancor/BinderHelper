...
void acquire_object(const sp<BinderHelper>& proc,
...
static void release_object(const sp<BinderHelper>& proc,
    const flat_binder_object& obj, const void* who, size_t* outAshmemSize)
{
    switch (obj.type) {
        case BINDER_TYPE_BINDER:
...
void release_object(const sp<BinderHelper>& proc,
    const flat_binder_object& obj, const void* who)
{
...
status_t flatten_binder(const sp<BinderHelper>& /*proc*/,
...
status_t unflatten_binder(const sp<BinderHelper>& proc,
...
{
    const sp<BinderHelper> proc(BinderHelper::self());
    status_t err;
...
{
    return flatten_binder(BinderHelper::self(), val, this);
}
...
status_t Parcel::writeValue(const Value& value) {
...
status_t Parcel::writeUniqueFileDescriptorVector(const std::unique_ptr<std::vector<unique_fd>>& val) {
...
restart_write:
        *reinterpret_cast<flat_binder_object*>(mData+mDataPos) = val;
        ...
        if (nullMetaData || val.binder != 0) {
            ...
            acquire_object(BinderHelper::self(), val, this, &mOpenAshmemSize);
            mObjectsSize++;
        }
...
status_t Parcel::writeMap(const Map& map_in)
{
    using ::std::map;
...
status_t Parcel::writeNullableMap(const std::unique_ptr<Map>& map)
{
    if (map == NULL) {
        return writeInt32(-1);
    }
...
status_t Parcel::readMap(Map* map_out)const
{
    using ::std::map;
...
status_t Parcel::readNullableMap(std::unique_ptr<Map>* map) const
{
...
{
    return unflatten_binder(BinderHelper::self(), *this, val);
}
...
{
    wp<IBinder> val;
    unflatten_binder(BinderHelper::self(), *this, &val);
    return val;
}
...
status_t Parcel::readValue(Value* value) const {
...
status_t Parcel::readUniqueFileDescriptorVector(std::unique_ptr<std::vector<unique_fd>>* val) const {
